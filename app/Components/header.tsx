...
export default function Header({ onSearch }: { onSearch?: (query: string) => void }) {
  ...
  const handleSearchClick = () => {
    if (search.trim()) {
      router.push(`/category/search?q=${encodeURIComponent(search)}`);
    }
  };
      ...
      <div className="flex justify-between items-center px-4 lg:ml-8 lg:mr-8">
        ...
        <div className="hidden lg:flex gap-4 items-center">
          <input
            ...
            className="search-bar pl-3 p-2 outline-none border border-gray-300 rounded-md focus:bg-custom-gray-medium"
          />
          <button onClick={handleSearchClick} className="hover:bg-custom-gray-medium p-2">
            <SearchIcon />
          </button>
          <Link href="/AdminLogin" className="hover:bg-custom-gray-medium p-2">Admin</Link>
          <Link href="/login" className="hover:bg-custom-gray-medium p-2">Login</Link>
          ...
          <ul className="flex flex-col gap-4 p-4">
            ...
            <li className="flex gap-2 items-center">
              <input
                ...
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search for products..."
                className="search-bar pl-3 p-2 border border-gray-300 rounded-md focus:bg-custom-gray-medium w-full"
              />
              <button onClick={handleSearchClick} className="hover:bg-custom-gray-medium p-2">
                <SearchIcon />
              </button>
